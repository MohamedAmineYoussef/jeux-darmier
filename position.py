...
class Position:
    ...
    def quatre_positions_diagonales(self):
        ...
        #TODO: À compléter
        diagonales = []
        diagonales.extend(self.positions_diagonales_bas())
        diagonales.extend(self.positions_diagonales_haut())
        return diagonales
...
if __name__ == '__main__':
    ...
    # TODO: À compléter
    position_test = Position(2, 2)
    result_bas = position_test.positions_diagonales_bas()
    assert len(result_bas) == 2
    position_test = Position(3, 4)
    result_bas = position_test.positions_diagonales_bas()
    assert len(result_bas) == 2
    position_test = Position(3, 3)
    result_bas = position_test.positions_diagonales_bas()
    assert len(result_bas) == 2
    print("Test positions_diagonales_bas bien réussi!")
    position_test = Position(2, 2)
    result_haut = position_test.positions_diagonales_haut()
    assert  len(result_haut) == 2
    position_test = Position(3,3)
    result_haut = position_test.positions_diagonales_haut()
    assert len(result_haut) == 2
    position_test = Position(4, 1)
    result_haut = position_test.positions_diagonales_haut()
    assert len(result_haut) == 2
    print("Test positions_diagonales_haut bien réussi!")
    result_quatre = position_test.quatre_positions_diagonales()
    assert len(result_quatre) == 4
    print("Test quatre_positions_diagonales bien réussi!")
    result_sauts = position_test.quatre_positions_sauts()
    assert len(result_sauts) == 4
    print("Test quatre_positions_sauts bien réussi!")
